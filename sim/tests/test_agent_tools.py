...
def test_parse_object_drive_status_moved_with_detection() -> None:
    summary = _parse_object_drive_status(
        ...
        "pub=3 cmd=19.0/17.0% heading_error=4.2deg det=sofa:florence-mlx:0.82 "
        "det_count=2 lock_rej=1 lock_gate=12.0deg lock_err=3.0deg pending=False lost=0\n",
        returncode=0,
    )
    ...
    assert summary["last_detection_count"] == 2
    assert summary["target_lock_reject_count"] == 1
    assert summary["last_target_lock_gate_deg"] == 12.0
    assert summary["last_target_lock_error_deg"] == 3.0
